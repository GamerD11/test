...
local realconfigs = {
    ...
    blockallremotes = false  -- Add this line
}
...
local function getFullPath(instance)
	...
	-- === Smart equipped tool detection ===
	-- If the instance is currently under a character, generate a stable character-based path
	do
		...
		if character then
			local charParts = {}
			current = instance
			while current and current ~= character and current ~= game do
				local name = current.Name
				local ref
				if isValidIdentifier(name) then
					ref = "." .. name
				else
					ref = formatLuaString(name)
				end
				table.insert(charParts, 1, ref)
				current = current.Parent
			end
			local base = 'game:GetService("Players").LocalPlayer.Character'
			return base .. table.concat(charParts, "")
		end
	end
	-- === End smart detection ===
	...
	local function getInstanceReference(inst)
	...
	while instance and instance ~= game do
		...
		local ref = getInstanceReference(instance)
		if ref then
			table.insert(pathParts, 1, ref)
		end
...
local foundSendFunctions = {}

local function formatLuaValue(val, indentLevel)
	local indent = string.rep("    ", indentLevel)

	if typeof(val) == "Instance" then
		local path = getFullPath(val)
		return "game" .. path:sub(5) -- convert to game path

	elseif typeof(val) == "Vector3" then
		return string.format("Vector3.new(%.6f, %.6f, %.6f)", val.X, val.Y, val.Z)

	elseif typeof(val) == "Vector2" then
		return string.format("Vector2.new(%.6f, %.6f)", val.X, val.Y)
	...
	elseif type(val) == "number" or type(val) == "boolean" or type(val) == "nil" then
		return tostring(val)

	elseif type(val) == "table" then
		-- Collect numeric keys and find max index
		local maxIndex = 0
		local hasNonNumericKey = false
		...
		if not hasNonNumericKey and maxIndex > 0 then
			-- Numeric table (possibly with gaps) → generate as normal array with nils filled in
			local result = "{\n"
			for i = 1, maxIndex do
				local v = val[i] -- will be nil if key doesn't exist
				result = result .. indent .. "    " .. formatLuaValue(v, indentLevel + 1) .. ",\n"
			end
			return result .. indent .. "}"
		else
			-- Has string keys or non-positive keys → use explicit [key] = style
			local result = "{\n"
			for k, v in pairs(val) do
				local keyStr = type(k) == "string" and string.format("[%q]", k) or "[" .. tostring(k) .. "]"
				result = result .. indent .. "    " .. keyStr .. " = " .. formatLuaValue(v, indentLevel + 1) .. ",\n"
			end
			return result .. indent .. "}"
		end
	end
	...
	return string.format("--[[unsupported: %s]]", typeof(val))
end

local function formatArgsTable(args)
    return "local args = " .. formatLuaValue(args, 0) .. "\n\n"
        .. 'game:GetService("ReplicatedStorage").ByteNetReliable:FireServer(unpack(args))'
end


-- 3. Create a separate function to log to SimpleSpy


-- 4. Find all send functions (unchanged)
for _, item in next, getgc(true) do
    if type(item) == "function" and islclosure(item) then
        local info = debug.getinfo(item)
        if info.name == "send" then
            table.insert(foundSendFunctions, item)
        end
    end
end
...
local Highlight = (isfile and loadfile and isfile("Highlight.lua") and loadfile("Highlight.lua")()) or loadstring(game:HttpGet("https://raw.githubusercontent.com/GamerD11/test/refs/heads/main/Highlight.lua"))()

local SimpleSpy3 = Create("ScreenGui",{ResetOnSpawn = false})
local Storage = Create("Folder",{})
local Background = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Color3.fromRGB(30, 30, 30),BackgroundTransparency = 0,Position = UDim2.new(0, 500, 0, 200),Size = UDim2.new(0, 450, 0, 268)})
...
local RightPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(45, 45, 48),BorderSizePixel = 0,Position = UDim2.new(0, 131, 0, 19),Size = UDim2.new(0, 319, 0, 249)})
Create("UICorner", {Parent = RightPanel, CornerRadius = UDim.new(0, 6)})

local CodeBox = Create("Frame",{Parent = RightPanel,BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(0, 319, 0, 119)})
...
local TopBar = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(0, 450, 0, 19)})
...
local selectedColor = Color3.fromRGB(88, 101, 242) -- Modern blue accent
local deselectedColor = Color3.fromRGB(128, 128, 128)
...
local originalUnreliableRemoteEvent = UnreliableRemoteEvent.FireServer
...
function newRemote(type, data)
    ...
    local log = {
        ...
        GenScript = data.genScript  -- Use the pre-generated script
    }
    ...
    local connect = Button.MouseButton1Click:Connect(function()
        ...
        -- No need to regenerate - use the cached script
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
    ...
    -- Modernized numeric array generation (fills missing indices with nil)
    local maxNum = 0
    local pureNum = true
    for kk,_ in next, t do
        if type(kk)=="number" and kk>0 and kk==math.floor(kk) then
            if kk > maxNum then maxNum=kk end
        else pureNum=false break end
    end
    if pureNum and maxNum>0 then
        for i=1,maxNum do
            size+=1
            if size > (getgenv().SimpleSpyMaxTableSize or 1000) then break end
            local val = t[i]
            local valStr = (val == nil) and "nil" or v2s(val, l, p, n, vtv, i, t, path.."["..i.."]", tables, tI)
            s = s .. "\n" .. string.rep(" ", l) .. "[" .. i .. "] = " .. valStr .. ","
        end
    else
...
function remoteHandler(data)
    -- Add blockallremotes check here
    if configs.blockallremotes then
        data.blocked = true
    end
    ...
    if not data.bypassBlacklist and (tablecheck(blacklist, data.remote, data.id) or data.remote.Name == "ByteNetReliable") then
        return
    end

    -- Generate the script immediately with current instance paths
    local gen = genScript(data.remote, data.args)
    if data.blocked or configs.blockallremotes then
        gen = "-- THIS REMOTE WAS PREVENTED FROM FIRING TO THE SERVER BY SIMPLESPY\n\n" .. gen
    end
...
local newindex = function(method,originalfunction,...)
    if typeof(...) == 'Instance' then
        ...
        if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") or remote:IsA("UnreliableRemoteEvent") then
            ...
            local blockcheck = tablecheck(blocklist,remote,id) or configs.blockallremotes  -- Add check here
            local args = {select(2,...)}
            ...
            if blockcheck then return end  -- This will block the remote call
        end
    end
            ...
            if IsA(remote,"RemoteEvent") or IsA(remote,"UnreliableRemoteEvent") or IsA(remote,"RemoteFunction") then    
                ...
                if not tablecheck(blacklist,remote,id) and not IsCyclicTable(args) then
                    ...
                    if configs.funcEnabled then
                        data.infofunc = info(2,"f")
                        local calling = getcallingscript()
                        data.callingscript = calling or cloneref(calling) or nil
                    end
...
local newUnreliableRemoteEvent = newcclosure(function(...)
    return newindex("FireServer",originalUnreliableRemoteEvent,...)
end)
...
local function disablehooks()
    if synv3 then
        ...
        unhook(Instance.new("UnreliableRemoteEvent").FireServer, originalUnreliableRemoteEvent)
        ...
        restorefunction(originalUnreliableRemoteEvent)
    else
        ...
        hookfunction(Instance.new("UnreliableRemoteEvent").FireServer, originalUnreliableRemoteEvent)
...
function toggleSpy()
    if not toggle then
        ...
        if synv3 then
            ...
            originalUnreliableRemoteEvent = hook(Instance.new("UnreliableRemoteEvent").FireServer, clonefunction(newUnreliableRemoteEvent))
            ...
            originalUnreliableRemoteEvent = hookfunction(Instance.new("UnreliableRemoteEvent").FireServer, clonefunction(newUnreliableRemoteEvent))
...
newButton("Run Code",
    function() return "Click to execute the GENERATED code shown above (best for GetChildren() paths & tools)" end,
    function()
        local code = codebox:getString()
        if not code or code == "" then
            TextLabel.Text = "No code in the box to execute"
            return
        end

        TextLabel.Text = "Executing generated code..."
        local func, loadErr = loadstring(code)
        if not func then
            TextLabel.Text = "Syntax error:\n" .. tostring(loadErr)
            return
        end
        ...
        if success then
            ...
            if result ~= nil then
                -- Try to copy return value if any
                pcall(function() setclipboard(v2s({result})) end)
            end
        else
            TextLabel.Text = "Runtime error while executing:\n" .. tostring(result)
        end
    end
)
...
newButton(
    ...
    function()
        ...
        -- Disconnect old connections and clear remoteLogs
        for _, v in next, remoteLogs do
            if typeof(v[1]) == "RBXScriptConnection" then
                pcall(function() v[1]:Disconnect() end)
            end
            if typeof(v[2]) == "Instance" then
                pcall(function() v[2]:Destroy() end)
            end
        end
        remoteLogs = {}
        clear(logs)
...
newButton(
    "Block All",
    function() 
        return string.format("[%s] Block all remotes from firing (still shows in UI)", 
            configs.blockallremotes and "ENABLED" or "DISABLED") 
    end,
    function()
        configs.blockallremotes = not configs.blockallremotes
        TextLabel.Text = string.format("[%s] Block all remotes from firing (still shows in UI)", 
            configs.blockallremotes and "ENABLED" or "DISABLED")
    end
)
...
local blacklist = {
    ["ByteNetReliable"] = true -- Add this line to prevent normal logging
}

-- Then modify the hookByteNetFunctions to bypass the blacklist
local function hookByteNetFunctions()
    for _, item in next, getgc(true) do
        if type(item) == "function" and islclosure(item) then
            local info = debug.getinfo(item)
            if info.name == "send" then
                ...
                old = hookfunction(item, function(...)
                    ...
                    -- Temporarily remove from blacklist for this logging
                    local wasBlacklisted = blacklist["ByteNetReliable"]
                    blacklist["ByteNetReliable"] = nil
                    
                    -- Create the formatted output
                    local formattedArgs = "local args = "..v2s(args).."\n\n"
                    local remotePath = "game:GetService(\"ReplicatedStorage\"):WaitForChild(\"ByteNetReliable\")"
                    local formattedOutput = formattedArgs..remotePath..":FireServer(unpack(args))"
                    
                    local logData = {
                        remote = game:GetService("ReplicatedStorage").ByteNetReliable,
                        method = "FireServer",
                        args = args,
                        callingscript = callingScript,
                        id = "ByteNet_"..tick(),
                        metamethod = "__namecall",
                        bypassBlacklist = true -- Add flag to bypass blacklist check
                    }
                    
                    -- Queue for SimpleSpy processing
                    schedule(remoteHandler, logData)
                    
                    -- Restore blacklist state
                    if wasBlacklisted then
                        blacklist["ByteNetReliable"] = true
                    end
